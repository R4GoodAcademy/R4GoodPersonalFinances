test_that("calculating effective tax rates", {
  ...
  test_asset_returns |> dplyr::glimpse()
  ...
  expect_equal(
    round(effective_tax_rates$effective_tax_rate, 4),
    c(0.2571, 0.2653, 0.2683, 0.2706, rep(0.4000, 5))
  )

  expect_true(
    all(
      -(effective_tax_rates$effective_tax_rate - 1) * 
        effective_tax_rates$expected_return == 
        effective_tax_rates$postliquidation_aftertax_expected_return
    )
  )
})
